...
This repository now also emits its own explicit `substrate-source/` folder. That generated substrate treats Workspace itself as a first-class XanaNode substrate so higher layers can federate with the Workspace repo instead of re-deriving Workspace facts ad hoc.
...
The current reference implementation is Node-based, but the protocol-facing contract should not require downstream tools to embed Node directly. Workspace is the reference behavior for a future stable `xananode-workspace` executable surface that Studio, Mobile, AI tools, and third-party applications can call from any stack.
...
Stable machine-facing aliases:

```bash
xananode-workspace health ./my-substrate --json
xananode-workspace export ./my-substrate --out ./dist --json
xananode-workspace snapshot ./my-substrate --message "Checkpoint" --json
```
...
For now, Workspace ships as a Node CLI and library. The intended long-term boundary is a stable `xananode-workspace` command contract that can later be packaged as a self-contained executable without changing what callers expect.

That executable surface should expose responsibilities equivalent to:
...
It should accept substrate folders, `.substrate` archives, and bundle JSON forms, then return structured machine-readable results so tools do not have to scrape prose output or reimplement Workspace rules.

Current CLI alias and output behavior:

- `status` has the stable alias `health`
- `pack` has the stable alias `export`
- `save` has the stable alias `snapshot`
...
Inside `XanaNode-Master`, Workspace can be packaged as a Windows executable launcher plus a sibling runtime tree:
...
```bash
npm run executables:build:workspace
```

This writes:

- `dist/win-x64/xananode-workspace.exe`
- `dist/win-x64/app/`
